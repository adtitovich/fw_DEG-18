...
echo "spark.jdbc.password=$(echo -n password | base64)" > credentials.properties
...
object fwnds {
  def main(args: Array[String]): Unit = {
    ...
    // считываем из файла в DataFrame
    def extracttoDataFrame(fileName: String, schema: StructType): Try[DataFrame] = {
      ...
      // столбцы, которые не будем использовать
      val dropColumn = Seq("total", "cogs", "grossmargin", "grossincome")
      ...
      try {
        val df = spark.read
          ...
          .schema(schema)
          .csv(fileName).toDF(newColumns: _*).drop(dropColumn: _*)
        Success(df)
      } catch {
    ...
    extracttoDataFrame(csvFile, schema) match {
      case Success(df) =>
      ...
      case Failure(ex) => println("Couldn't create dataframe")
    }
